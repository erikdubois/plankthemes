...
find `pwd` -mindepth 1 -maxdepth 1 -type d | egrep -v '.git' | awk --field-separator='/' '{ print $0 "\t\t/usr/share/plank/themes/" $6 }' | awk --field-separator='\t\t' $'{ print "ln -s $\'" $1 "\' " "$\'" $2 "\'"  } ' > install-plank-theme-links.sh
...
find `pwd` -mindepth 1 -maxdepth 1 -type d | egrep -v '.git' | awk --field-separator='/' '{ print $0 "\t\t/usr/share/plank/themes/" $6 }' | awk --field-separator='\t\t' $'{ print "unlink $\'" $2 "\'"  } ' > uninstall-plank-theme-links.sh
